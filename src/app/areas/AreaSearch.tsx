...
import s from './AreaSearch.module.scss'
...
export default function AreaSearch({ defaultValue = '' }: { defaultValue?: string }) {
  ...
  const fetchSuggestions = useCallback(async (q: string) => {
    ...
    try {
      ...
      const items: SuggestionItem[] = (json.data ?? []).map((a: {
        ...
        slug: a.pageUrl?.url?.replace(/^\/areas\//, '').replace(/\/$/, '') ?? String(a.id),
        name: a.title,
      }))
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault()
      router.push(`/areas/${suggestions[activeIdx].slug}`)
      setOpen(false)
