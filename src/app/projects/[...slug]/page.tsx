...
export async function generateStaticParams() {
  ...
  return slugs
    ...
    .map((s) => ({
      slug: s.replace('/projects/', '').replace(/\/$/, '').split('/'),
    }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const pageSlug = `/projects/${slug.join('/')}/`
  ...
  return {
    ...
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_SITE_URL}/projects/${slug.join('/')}`,
    },
  }
}
...
function renderBlock(block: PenthouseBlock, index: number, page: PenthousePage, entityId?: number) {
  ...
  switch (block.__component) {
    ...
    case 'block.developers': {
      ...
      const stats = [
        ...(yearsOnMarket ? [{ label: 'Years on the market', value: `${yearsOnMarket}+` }] : []),
        ...(dev.completedProjectsCount ? [{ label: 'Completed projects', value: String(dev.completedProjectsCount) }] : []),
        ...(dev.underConstructionProjectsCount ? [{ label: 'Under construction', value: String(dev.underConstructionProjectsCount) }] : []),
      ]
