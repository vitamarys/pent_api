...
import s from './AgentSearch.module.scss'
...
export default function AgentSearch({ defaultValue = '' }: { defaultValue?: string }) {
  ...
  const fetchSuggestions = useCallback(async (q: string) => {
    ...
    try {
      ...
      const res = await fetch(`${base}/api/catalog/agents?search=${encodeURIComponent(q)}&pageSize=8`)
      ...
      const items: SuggestionItem[] = (json.data ?? []).map((a: {
        ...
        slug: a.pageUrl?.url?.replace(/^\/agents\//, '').replace(/\/$/, '') ?? String(a.id),
        name: a.name,
      }))
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault()
      router.push(`/agents/${suggestions[activeIdx].slug}`)
      setOpen(false)
