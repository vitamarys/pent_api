...
import s from './PopPresentation.module.scss';
...
export interface PopPresentationProps {
  open:      boolean;
  onClose:   () => void;
  image?:    string;
  title?:    string;
...
export default function PopPresentation({
  open,
  onClose,
  image,
  title,
    ...
    <div className={s.backdrop} onClick={onClose}>
      <div className={s.modal} onClick={e => e.stopPropagation()}>
        ...
        <div className={s.header}>
          <span className={s.headerLabel}>Presentation</span>
        ...
        <div className={s.body}>
          ...
          <div className={s.formPanel}>
            <div className={s.intro}>
              <h2 className={s.title}>Get professional property guidance</h2>
              <p className={s.desc}>
                Explore all features, layouts, and specifications in one file
              </p>
            </div>
            ...
            <form className={s.form} onSubmit={handleSubmit(submit)} noValidate>
              ...
              <div className={s.submitArea}>
                {isSubmitSuccessful ? (
                  <p className={s.successMsg}>Thank you! We&apos;ll send the presentation to your email.</p>
                ) : (
                  <button className={s.submitBtn} type="submit" disabled={isSubmitting}>
                    {isSubmitting ? 'Sending…' : 'Download presentation'}
                  </button>
                )}
